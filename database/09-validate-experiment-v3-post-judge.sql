-- ============================================================
-- Experiment 003: Post-Judge validation
-- ============================================================
--
-- Purpose:
--   Run these checks AFTER the Judge run for HRA-EXP-V3 completes.
--   All queries should return zero/expected values.

-- ============================================================
-- 1. All 123 pairs have reference annotations
-- ============================================================
...
-- ============================================================
-- 4. Score arithmetic: role + skills + experience + conditions = total score
-- ============================================================
...
-- ============================================================
-- 6. Distribution of decisions looks plausible
-- ============================================================
...
-- ============================================================
-- 7. Distribution of new hard-negative cases (should be mostly no_match)
-- ============================================================

SELECT
    'New hard-negative candidates: mostly no_match expected' AS check_name,
    ...
    COUNT(*) AS total,
    ROUND(100.0 * SUM(CASE WHEN cv.reference_decision = 'no_match' THEN 1 ELSE 0 END) / COUNT(*), 2) AS no_match_pct
FROM eval_prompt_case_vacancies cv
JOIN eval_prompt_cases c ON cv.case_id = c.id
JOIN eval_prompt_datasets d ON c.dataset_id = d.id
WHERE d.dataset_code = 'HRA-EVAL-V3'
  AND c.case_code BETWEEN 'HRA-EVAL-V2-000101' AND 'HRA-EVAL-V2-000111'
  AND cv.reference_decision IS NOT NULL;
...
-- ============================================================
-- 10. Summary row for quick PASS/FAIL
-- ============================================================
...
SELECT
    ...
    (
        (SELECT COUNT(*) FROM eval_prompt_case_vacancies cv
         JOIN eval_prompt_cases c ON cv.case_id = c.id
         JOIN eval_prompt_datasets d ON c.dataset_id = d.id
         WHERE d.dataset_code = 'HRA-EVAL-V3' AND cv.reference_score IS NULL) = 0
        AND
        (SELECT COUNT(*) FROM eval_prompt_case_vacancies cv
         JOIN eval_prompt_cases c ON cv.case_id = c.id
         JOIN eval_prompt_datasets d ON c.dataset_id = d.id
