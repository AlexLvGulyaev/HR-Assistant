...
Единственное место истории изменений каждого документа. Дисциплина: дата «Последнее обновление» в футере документа совпадает с датой его последней записи в этой таблице. Детальные протоколы работ хранятся вне публичного репозитория; PROJECT_STATE — паспорт проекта и вехи, а не история изменений документов.

| Дата | Документы | Что изменено |
|------|-----------|--------------|
| 2026-09-16 | 13 документов finetuning/ | Причёсывание под стандарт APL: H2 «эмодзи + номер + название» на всех документах; футеры-триады (Статус / Последнее обновление / История изменений); штамп-проход («ключевой», «полноценный», «основной» без контрастного множества); заголовок и упоминания внутреннего артефакта в реестре визуальных артефактов нейтрализованы |
| 2026-09-16 | внутренние повествовательные спецификации (finetuning/, finetuning/landing/) | Выведены из публичного репозитория (gitignore); упоминания в публичных документах заменены нейтральной формулировкой |
| 2026-09-16 | PROJECT_STATE | Рефакторинг по модели «паспорт + вехи» (523→218 строк): §2 — дубль таблицы Production Readiness удалён, Known Issues → компакт со ссылкой на known-issues.md (SOT), открытые вопросы ML-контура — однострочниками; §3 Documentation Status — 124→12 строк (актуальное состояние, SOT-аудит 2026-06-24 → веха); §8 Next Steps — 168→30 строк (открытые шаги; протоколы Phase 0b/0c/1/2 — вехи в Status History, детали в профильных документах); §9 Documentation Roadmap (выполнен) удалён, перенумерация §10→9, §11→10; упоминание внутреннего артефакта в вехе заменено нейтральным. CHANGE_LOG §4: упоминание внутреннего артефакта заменено нейтральной формулировкой |
| 2026-09-16 | INTEGRATION_DIAGRAM, DEPLOYMENT_GUIDE §10.2, SUPPORT_RUNBOOK §8 | SSOT-дедуп однородных документов: INTEG §2 — системный граф заменён ссылкой на ARCHITECTURE §2; INTEG §5 — таблица таблиц заменена ссылками на database/README (SOT) и ARCHITECTURE §4; INTEG §7 (ER-дубль) удалён, перенумерация §8→7, §9→8; DG §10.2 — SQL-дубль → ссылка на RUNBOOK §7 (SOT операционных SQL); RUNBOOK §8 — pg_dump-дубль → ссылка на DG Шаг 11 (SOT настройки бэкапов) |
...
| 2026-09-03 | finetuning/landing/DEMO_ROUTE | Создан маршрут демонстрации LoRA-лендинга |
| 2026-07-27 | 10 документов finetuning/ | Пакет документации finetuning и лендинга v3: ARCHITECTURE, TECHNICAL_FOUNDATION, отчёты Experiments 001–004, external_validation_report; лендинг — README, DEPLOYMENT_GUIDE, LANDING_ARCHITECTURE, реестр визуальных артефактов |
