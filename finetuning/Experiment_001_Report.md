...
Датасет `HRA-EVAL-V1` включён в репозиторий в каталоге [`data/`](data/). Все профили в нём синтетические: HR Assistant никогда не работал в реальном боевом режиме. Первичные артефакты обучения (weights, raw metrics, evaluation JSON, operation logs) хранятся в закрытом рабочем контуре и не публикуются.

---

**Статус:** Технический отчёт Experiment 001
